...
# %%
#Create the final layer with two final output classes (Binary classifier) 
import torch
import torchvision.models as models
...
# %%
#evaluate the weights, this is the best weights my model could create
...
with torch.no_grad():
    for ind, (inputs, labels) in enumerate(eval_loader):
        ...
        total += labels.size(0)
        correct += (predicted == labels).sum().item()
        print(f"Batch {ind+1}: Accuracy: {(predicted == labels).sum().item() / 50}")
